...
<<lookatdata>>=
dims <- lapply(mget(ls()), dim)
outer(dims, dims, Vectorize(all.equal))
rm(dims)
rnames <- lapply(mget(ls()), rownames)
outer(rnames, rnames, Vectorize(all.equal))
rm(rnames)
cnames <- lapply(mget(ls()), colnames)
outer(cnames, cnames, Vectorize(all.equal))
rm(cnames)
...
We that the governing feature of the most sensitivity-discordant genes is that RNA-Seq explains the vast majority of change in gene expression. To see what a row-linear fit looks like for one of these genes, we again use \texttt{plotOneFit}. 
